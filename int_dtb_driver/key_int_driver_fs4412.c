...
{
    printk("key 3 is pressed\n");
    fs4412_beep_off();
    return IRQ_HANDLED;
}

//在probe接口中实现驱动的初始化操作
int platform_key_probe(struct platform_device *pdev)  //探测函数接口
{
    int i = 0;
    int ret = 0;
    printk("platform key probe success \n");
    for(;i<pdev->num_resources;i++)
    {
        printk("resource[%d].start~end is %p~%p,int_flag is %x\n ",i,pdev->resource[i].start,pdev->resource[i].end,pdev->resource[i].flags);

    }
    //注册中断处理函数
    ret = request_irq(pdev->resource[2].start,key2_handler,pdev->resource[2].flags,"key2",NULL);
    ret = request_irq(pdev->resource[3].start,key3_handler,pdev->resource[3].flags,"key3",NULL);
